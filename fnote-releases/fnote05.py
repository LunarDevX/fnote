...
class fnote:
    ...
    def add_note(self):
        ...
        date = self.get_date()
        id_ = self.generate_id(self.get_last_id(self.table_name))

        if answer == "y":
            self.insertVariableIntoTable(self.table_name, id_, title, date, tag, note)
    ...
    def search_for_tag(self):
        ...
        draw_list = self.search_tag_table(self.table_name, tag)
    ...
    def search_tag_table(self, table_name, search_tag):
        ...
        try:
            ...
            for row in cursor.execute("SELECT * FROM overview;"):
                ...
                tmp_tag = tag.lower()
                result = re.findall(search_tag, tmp_tag)
                if len(result) != 0:
    ...
    def delete_note(self):
        ...
        if self.check_id_in_database(self.table_name, note_id):
            print(self.hyphen)
            print("Do you want to delete this note? (y/n)")
            answer = input().lower().strip()
            if answer == "y":
                self.delete_note_database(self.table_name, note_id)
        else:
            print(self.hyphen)
    ...
    def delete_note_database(self, table_name, table_id):
    ...
    def insertVariableIntoTable(self, table_name, id_, title, date, tag, note):
    ...
    def edit_note(self):
        ...
        if self.check_id_in_database(self.table_name, note_id):
            ...
            title = self.get_title_database(self.table_name, note_id)
            note = self.get_note_database(self.table_name, note_id)
            tag = self.get_tag_database(self.table_name, note_id)
            ...
            if answer == "y":
                self.update_database(
                    self.table_name, new_title, new_note, new_tag, note_id
                )
        else:
            print()
            print("Note does not exist.")
    ...
    def check_id_in_database(self, table_name, table_id):
    ...
    def update_database(self, table_name, new_title, new_note, new_tag, id_):
    ...
    def get_title_database(self, table_name, table_id):
    ...
    def get_note_database(self, table_name, table_id):
        """Get the current title from the database overview."""
    ...
    def get_tag_database(self, table_name, table_id):
        """Get the current title from the database overview."""
